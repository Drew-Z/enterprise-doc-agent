...
**本分支的判定与前置条件改进仍为候选，未部署。** `presales.v5` 完成 18 次单次模型请求，无重试：15 条中文草稿分类匹配，另有 1 次超时、2 次连接失败；29 处引用逐字有效，必要证据覆盖 25/30。逐条审阅仍发现正文写反条款关系、将未知验收状态写成未完成等问题，未达到发布门槛。这是完整合成证据的生成阶段验证，尚不代表公网端到端或真实客户准确率。见[候选结果与失败记录](docs/ops/presales-quality-evaluation.md#判定顺序候选-v5未发布)。

后续 v6 提示词试验已撤回：首组六次仅接受三条草稿，另有连接错误、无效引用编号和前提一致性错误，未知状态误判仍存在。按预定规则停止其余18次请求；保留脱敏诊断和[全部结果](docs/ops/presales-quality-evaluation.md#正文事实候选-v6已撤回)，未变更线上版本。
