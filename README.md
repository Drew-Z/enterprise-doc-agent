...
**本分支 v7 为未部署候选。** 模型只接收本次引用编号与必要来源信息，展示条件由结构化前提统一生成。数据库和浏览器流程通过本地验证；六次真实模型请求得到四条分类正确的中文草稿、五处有效原文引用，另外两次达到120秒上限。按预定规则停止其余18次调用，条款冲突与未知验收状态仍待验证，尚未达到发布门槛。见[协议改进与完整结果](docs/ops/presales-quality-evaluation.md#单一来源协议候选-v7未发布)。
