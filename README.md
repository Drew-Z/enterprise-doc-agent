...
**当前阶段：公开演示与受邀公网试点。** 2026-09-23 已部署 [v0.1.43](https://github.com/Drew-Z/enterprise-doc-agent/tree/v0.1.43)，可以免登录进入独立演示企业。正式企业使用 GitHub 登录，访问需要准入或邀请。当前采用 4 核 4 GB 单节点，数据库、对象存储和模型服务外置，适合低并发试用。

**公开演示已通过真实公网流程验收。** 两份 TXT 批量上传、解析、外部模型生成、六处原文引用核查、三条人工复核和已复核 CSV 导出全部通过；刷新恢复、手机布局、第二访客隔离及退出也已验证。三次生成均成功、没有重试，分别耗时约 52、54、70 秒；模型时延仍有波动。详见[本次部署](https://github.com/Drew-Z/enterprise-doc-agent/actions/runs/35831689248)与[演示手册](docs/ops/public-pilot-runbook.md#公开演示企业)。

**复杂采购资料仍需改善判断质量。** 随后的两轮公网复测共 12 次请求，得到 10 条有效草稿、7 条正确分类，成功响应中位数约 74 秒；失败与错误分类均已保留。现提供[可复现评测工具、原始结果和问题说明](docs/ops/presales-quality-evaluation.md)，回答仍需人工核对。
...
- [ ] 验证判定规则修正，扩充独立保留集与真实客户资料，改善生成时延并核实实际费用。
