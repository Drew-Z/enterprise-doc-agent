...
**本分支的前置条件改进仍为候选，未部署。** `presales.v4` 的 12 次单次模型调用中，初始解码接受 10 条；修正重复引用要求后，原始响应离线重放得到 12 条中文草稿、分类匹配 10/12。已知 H1 回归通过，但新 H2 样例仍混淆“证据不足 / 资料冲突”与“不满足”，尚未达到发布门槛。这是完整合成证据的生成阶段验证，不是新一轮公网流程验收。见[候选结果](docs/ops/presales-quality-evaluation.md#前置条件候选-v4未发布)。
