...
def test_recovery_dependencies_use_isolated_volumes_and_loopback_ports() -> None:
    ...
    assert services["recovery-redis"]["ports"] == [
        "127.0.0.1:${RECOVERY_REDIS_PORT:-16379}:6379"
    ]
    assert services["recovery-redis"]["image"].startswith(
    ...
    assert services["recovery-redis"]["volumes"] == [
        "enterprise-doc-recovery-redis-data:/data"
    ]
    assert services["recovery-minio"]["volumes"] == [
        "enterprise-doc-recovery-minio-data:/data"
    ]
