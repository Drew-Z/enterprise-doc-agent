...
- Restricted-network image relay receipts bind the archive checksum, immutable deployment
  image reference, short receiver base name and `scripts/import_staging_oci_archive.py`. The
  receiver is dry-run-first, validates OCI metadata without extraction, rewrites only a
  temporary import index so every validated image descriptor is an anonymous root, imports
  with the fully qualified `docker.io/library/<relay-id>` base, and inspects canonical digest
  aliases for every image index and manifest. It then binds and target-verifies the exact
  deployment reference. Importing the named relay archive directly with `--base-name` is
  invalid because containerd filters out the original root name and creates no canonical
  records.
