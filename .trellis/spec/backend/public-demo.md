...
Provision entitlement `created_at` and `updated_at` explicitly from the same clock as its period. Migration `20260914_0026` makes these columns required without server defaults; the ORM metadata defaults are not the deployed schema contract. Guest creation regression must rebuild the entitlement tables using that migration, then exercise `DemoService.start/get`, so metadata-only fixtures cannot conceal this difference.
