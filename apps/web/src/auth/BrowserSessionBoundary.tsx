...
export function BrowserSessionBoundary({ children, entry }: { children: (session: BrowserWorkspaceSession) => ReactNode; entry?: AuthEntry }) {
  ...
  }, [state.phase, state.notice, state.session?.contextVersion]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const invitationLink = () => {
      ...
      const authenticated = snapshot.session !== null && ["choosing", "working", "busy"].includes(snapshot.phase);
  ...
  if (state.phase === "working" && state.session?.currentTenant && !invitationOpen) {
    return <Fragment key={state.workspaceKey}><div hidden={state.verifying} inert={state.verifying}>{children({ tenant: state.session.currentTenant, email: state.session.email, chooseTenant: () => { void controller.showTenants(); }, logout: () => { void controller.logout(); } })}</div>{state.verifying && <main className="browser-entry"><div className="browser-wait" role="status"><LoaderCircle className="spin" aria-hidden="true" /><h2>{zh ? "正在核验，请稍候" : "Checking your session"}</h2></div></main>}</Fragment>;
  }
  const notice = state.notice ?? (entry?.signInFailed ? "sign_in_failed" : null);
  return <main className="browser-entry">
    ...
    <div className="browser-entry-layout">
      ...
      <section className="browser-entry-card" aria-label={zh ? "企业登录" : "Enterprise sign-in"}>
        ...
        {state.phase === "anonymous" && <><span className="browser-card-icon"><Building2 aria-hidden="true" /></span><h2>{zh ? "登录工作台" : "Sign in to your workspace"}</h2><p>{zh ? "使用企业账号登录，然后选择你有权访问的企业。" : "Sign in with your company account, then select an authorized enterprise."}</p>{entry?.admissionLink && <p className="browser-notice">{zh ? "请先登录，再重新打开原开通链接。" : "Sign in first, then reopen your admission link."}</p>}<a className="browser-primary" href="/auth/login" onClick={() => controller.suspend()}>{zh ? "使用企业账号登录" : "Sign in with your company account"}<ArrowRight aria-hidden="true" /></a></>}
