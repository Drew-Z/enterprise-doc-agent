...
const attempt = z.object({ id, number: z.number().int().positive(), state: z.enum(["running", "succeeded", "failed", "expired"]), errorCode: z.string().nullable(), modelProvider: z.string(), modelName: z.string().nullable(), providerRequestCount: z.number().int().min(0).max(1).nullable(), provenance: z.record(z.string(), z.string().nullable()), usage: z.record(z.string(), z.number().int().nonnegative().nullable()).nullable(), createdAt: time, finishedAt: time.nullable(), deadlineAt: time }).strict();
...
const row = z.object({ id, requirement, revision: z.number().int().nonnegative(), state: z.enum(["pending", "running", "drafted", "failed"]), draft: draft.nullable(), review: review.nullable(), reviewHistory: z.array(review), attempts: z.array(attempt).max(3) }).strict();
...
export const packetSchema = packetSummarySchema.extend({ sources: z.array(source).min(1).max(6), rows: z.array(row).min(1).max(12) });
